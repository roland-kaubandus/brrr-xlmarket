  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="font-normal text-[#94A3B8]"> · {children.length}</span>
      </h2>
      <div className="relative max-w-[1360px] mx-auto">
        {canLeft && (
          <button
            type="button"
            onClick={() => scrollBy(-1)}
            aria-label={locale === "et" ? "Keri vasakule" : "Scroll left"}
        ...
        {canRight && (
          <button
            type="button"
            onClick={() => scrollBy(1)}
            aria-label={locale === "et" ? "Keri paremale" : "Scroll right"}
        ...
        {/* Serva-gradient (Variant 1): discoverability-vihje, et rida keritav — nähtav
            KÕIGIL viewport'idel (ei md:-gated, sest mobiilis nooled peidus). pointer-events-none. */}
        {canLeft && (
          <div
            aria-hidden
            className="pointer-events-none absolute left-0 top-0 bottom-3 w-8 sm:w-10 z-[5] bg-gradient-to-r from-white to-transparent"
          />
        )}
        {canRight && (
          <div
            aria-hidden
            className="pointer-events-none absolute right-0 top-0 bottom-3 w-8 sm:w-10 z-[5] bg-gradient-to-l from-white to-transparent"
          />
        )}
      <div
        ref={trackRef}
        role="list"
        aria-label={locale === "et" ? "Alamkategooriad" : "Subcategories"}
