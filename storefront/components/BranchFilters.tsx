...
export default function BranchFilters({
  ...
  const sortOptions = [
    { value: "uusimad", label: locale === "et" ? "Uusimad" : "Newest" },
    { value: "odavamad", label: locale === "et" ? "Odavamad" : "Cheapest" },
    { value: "kallimad", label: locale === "et" ? "Kallimad" : "Most Expensive" },
  ]
    ...
    <div className="mb-6">
      ...
      <div className="flex items-center gap-0 rounded-2xl border border-soft-border bg-white overflow-hidden h-[48px]">
        ...
        <div className="flex items-center gap-1 px-4 border-r border-soft-border h-full">
          <span className="text-xs font-medium text-muted mr-1 hidden sm:inline">{locale === "et" ? "Sorteeri" : "Sort"}</span>
          {sortOptions.map((opt) => (
            <button
              key={opt.value}
              type="button"
        ...
        <div className="ml-auto px-4 text-xs text-muted hidden md:flex items-center h-full">
          {totalProducts.toLocaleString("en")} {locale === "et" ? "toodet" : "products"}
        </div>
      </div>
      ...
      {drawerOpen && (
        <div className="mt-3 p-5 rounded-2xl border border-soft-border bg-white animate-in slide-in-from-top-2 duration-200">
          <form onSubmit={handlePriceApply} className="flex flex-wrap items-end gap-4">
            <div>
              <label className="block text-xs font-medium text-muted mb-1.5">{locale === "et" ? "Min hind" : "Min price"}</label>
              <input
                type="number"
                value={minPrice}
            ...
            <div>
              <label className="block text-xs font-medium text-muted mb-1.5">{locale === "et" ? "Max hind" : "Max price"}</label>
              <input
                type="number"
                value={maxPrice}
            ...
            {hasFilters && (
              ...
              >
                {locale === "et" ? "Tühista kõik" : "Clear All"}
              </button>
            )}
          </form>
          ...
          {hasFilters && (
            <div className="flex flex-wrap items-center gap-2 mt-4 pt-4 border-t border-soft-border">
              <span className="text-[10px] uppercase tracking-[0.14em] text-muted">{locale === "et" ? "Aktiivne:" : "Active:"}</span>
              {currentSort && (
