...
export default function AccountPage() {
  ...
  return (
    <div className="max-w-[900px] mx-auto px-4 py-10">
      ...
      {/* Addresses */}
      <div className="border border-[#E2E8F0] rounded-xl p-6 mb-6">
        <h2 className="text-lg font-semibold text-[#1E293B] mb-4">Saved Addresses</h2>
            ...
            {customer.addresses.map((addr) => (
              <div key={addr.id} className="p-4 bg-[#F8FAFC] rounded-lg text-sm">
                ...
                {addr.is_default_shipping && (
                  <span className="inline-block mt-2 text-xs text-[#D97706] font-medium">Default shipping address</span>
                )}
              </div>
            ))}
          </div>
        ) : (
          <p className="text-sm text-[#94A3B8]">No saved addresses.</p>
        )}
      </div>
