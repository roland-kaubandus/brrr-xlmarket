    ...
    <div className="max-w-[1280px] mx-auto px-[16px] sm:px-[24px] py-[32px] sm:py-[48px]">
      <div className="flex items-baseline justify-between mb-[24px]">
        <div>
          ...
          {query && (
            <p className="text-[14px] font-[family-name:var(--font-inter)] text-[#999999]">
        ...
        {query && totalHits > 0 && (
          <div className="hidden sm:flex items-center gap-[8px]">
            <span className="text-[12px] text-[#999999] font-[family-name:var(--font-inter)]">Sordi:</span>
            <div className="flex gap-[4px]">
              {SORT_OPTIONS.map(opt => (
                <Link
                  key={opt.value}
                  href={`/otsing?q=${encodeURIComponent(query)}&sort=${opt.value}`}
                  className={`px-[10px] py-[4px] text-[12px] font-[family-name:var(--font-inter)] transition-colors ${
        ...
        <>
          ...
          {totalPages > 1 && (
            <nav className="flex justify-center items-center gap-[8px] mt-[40px]">
              {page > 1 && (
                <Link
                  href={`/otsing?q=${encodeURIComponent(query)}&leht=${page - 1}${currentSort ? `&sort=${currentSort}` : ""}`}
