...
```
DATABASE_URL=postgres://xlmarket:PG_PASSWORD_REDACTED@localhost:5435/xlmarket
```
...
ADMIN_PASS=MEDUSA_ADMIN_PASSWORD_REDACTED
```
...
curl -s -X POST http://127.0.0.1:9001/auth/user/emailpass \
  ...
  -d '{"email":"tarmo@xlmarket.eu","password":"MEDUSA_ADMIN_PASSWORD_REDACTED"}' | jq -r '.token'
