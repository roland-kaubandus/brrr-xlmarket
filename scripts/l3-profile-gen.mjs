...
const BASE = OUT === "full" ? "l3-profiilid" : "l3-profiilid-" + OUT;
const BATCH = 6;
const SAMPLE = 7;
...
// TEADLIKUD UMBRELLAD (faas2-umbrella-teadlik.md) — need saavad ALATI umbrella:true
const UMBRELLA = new Set();
try { const t = fs.readFileSync("/opt/eumotors-tasks/reports/faas2-umbrella-teadlik.md", "utf8"); for (const m of t.matchAll(/\((pcat_[a-z0-9_]+)/g)) UMBRELLA.add(m[1]); } catch {}
// WHITELIST-NAABRID (merge-whitelist.json) — id → [naaber-nimi, ...]
const WL = {};
try { const w = JSON.parse(fs.readFileSync("./scripts/merge-whitelist.json", "utf8")); for (const p of w.pairs || []) { (WL[p.a] ||= []).push(p.b_name); (WL[p.b] ||= []).push(p.a_name); } } catch {}
console.error(`Umbrellad: ${UMBRELLA.size} · whitelist-paarid: ${Object.keys(WL).length} L3.`);
...
// RESUME: kui väljund-JSON olemas, jäta juba tehtud L3-d vahele (jätka poolelt)
const OUTJSON = `/opt/eumotors-tasks/reports/${BASE}.json`;
let doneProfiles = [];
if (!args.includes("--force")) { try { doneProfiles = JSON.parse(fs.readFileSync(OUTJSON, "utf8")).profiles || []; } catch {} }
const doneIds = new Set(doneProfiles.map((p) => p.l3_id));
const skip = rows.filter((r) => doneIds.has(r.id)).length;
rows = rows.filter((r) => !doneIds.has(r.id));
console.error(`Profiile: ${rows.length} L3 (${skip} juba tehtud, jätkan), ${Math.ceil(rows.length / BATCH)} API-kutset.`);

// näidised + naaber-L3 nimed (sama L2) + umbrella/whitelist märgend
for (const r of rows) {
  r.isUmbrella = UMBRELLA.has(r.id);
  r.wl = WL[r.id] || [];
...
- **UMBRELLA:** kui L3 sisend on märgitud "TEADLIK UMBRELLA" VÕI sisu näitab selgelt mitut alamtüüpi koos (nt 'Arkaadimängud'=skeeball+korvpall+claw), pane "umbrella": true + "umbrella_pohjus" (miks koos hoitakse) ja domeen-lauses "[UMBRELLA]". INCLUDE peab katma KÕIK alamtüübid; EXCLUDE lisab "ÄRA splitti alamtüüpideks — teadlik umbrella". Muidu "umbrella": false.
- **WHITELIST-NAABER:** kui sisend märgib "WHITELIST-NAABER: X", lisa naaber_piirid'i "EI merge naabriga X (whitelistitud, eri väljund)".
...
[{"l3_id":"...","umbrella":true|false,"umbrella_pohjus":"kui umbrella, miks koos; muidu tühi","domeen":"funktsionaalne domeen + väljund, 1 lause eesti (märgi [UMBRELLA] kui on)","include":["eesti / english / polski", ...],"exclude":["tüüp / keyword → Õige-L3", ...],"naaber_piirid":["piiripealse suunamine + whitelist-naabrid", ...],"valjundi_test":"1 lause: mis eristab naabrist"}]`;

async function genBatch(batch) {
  const blocks = batch.map((r) => `### L3 id=${r.id} — nimi="${r.name}" — main="${r.main}" — L2="${r.l2}" (${r.n} toodet)${r.isUmbrella ? "\n⚑ TEADLIK UMBRELLA (märgi umbrella:true, ÄRA soovita splitti)" : ""}${r.wl.length ? `\n⚑ WHITELIST-NAABER: ${r.wl.join(", ")} (märgi naaber_piirid: EI merge)` : ""}\nNAABER-L3-d samas L2: ${r.naabrid.join(", ") || "(pole)"}\nNäidistooted:\n${r.titles.join("\n")}`).join("\n\n");
...
for (let i = 0; i < rows.length; i += BATCH) {
  ...
  } catch (e) { console.error(`  batch ${i} VIGA: ${e.message}`); batch.forEach((r) => out.push({ l3_id: r.id, l3_nimi: r.name, main: r.main, l2: r.l2, n: r.n, umbrella: false, umbrella_pohjus: "", domeen: "ERROR: " + e.message.slice(0, 80), include: [], exclude: [], naaber_piirid: [], valjundi_test: "" })); }
  fs.writeFileSync(OUTJSON, JSON.stringify({ generated: new Date().toISOString().slice(0, 10), model: MODEL, count: out.length, profiles: out }, null, 1)); // crash-safe inkrementaalne salvestus
}

fs.writeFileSync(`/opt/eumotors-tasks/reports/${BASE}.json`, JSON.stringify({ generated: new Date().toISOString().slice(0, 10), model: MODEL, count: out.length, profiles: out }, null, 1));
const md = [`# L3 TÜÜBI-PROFIILID — ${OUT}\n`, `**${new Date().toISOString().slice(0, 10)} · ${MODEL} · ${out.length} L3**\n`];
for (const p of out) {
  md.push(`## ${p.l3_nimi}  \`${p.l3_id}\`  (${p.main} / ${p.l2}, ${p.n} toodet)${p.umbrella ? "  🌂 UMBRELLA" : ""}`);
  if (p.umbrella) md.push(`**Umbrella:** ${p.umbrella_pohjus || "teadlik umbrella — feed mapib alamtüübid, ära splitti"}`);
...
fs.writeFileSync(`/opt/eumotors-tasks/reports/${BASE}.md`, md.join("\n"));
const cost = (USAGE.in / 1e6) * 5 + (USAGE.cache_r / 1e6) * 0.5 + (USAGE.cache_w / 1e6) * 6.25 + (USAGE.out / 1e6) * 25;
console.log(`\n🟢 ${out.length} profiili · raport: reports/${BASE}.{md,json}`);
