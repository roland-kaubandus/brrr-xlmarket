...
### P2. ✅ SULETUD (2026-09-20) — FAAS 1 SAMM 3: Sisu-generaator backfill 100%
- **Seis oli:** täis-batch-jooks (2026-08-25) kukkus krediidi otsasaamisel ~29% → NULL 5347.
- **✅ RE-RUN VALMIS 2026-09-20:** `bash scripts/run-content-backfill.sh` → 5346/5347 ok, 0 krediidi-viga (~$171). Viimane 1 (`prod_01KNXXNXC53QZEVBWS1P2VTXQN`, N3-truncation) healed truncation-retry'ga. **DB nüüd: hash OK 18675 / korpus 18675 = KATE 100%, PUUDUB 0** (kontroll `filterNeedsGen` SSoT-iga).
- **✅ STRUKTUURNE FIX — N3 truncation-retry JAGATUD teel (HARD RULE #5):** `content-gen.mjs` `generateContent` → kui vastus katkeb `stop_reason=max_tokens`-lakkes, tõsta lagi 8000→`RETRY_MAX_TOKENS`=16000 + proovi ÜKS kord uuesti (ei kuluta HTTP-retry eelarvet). Kaitseb (a) **öist hooki [6.5]** (`pipeline-content-gen.mjs` → generateContent) JA (b) **batch-backfilli** (truncatud tulemused healitakse sama jagatud generateContent-teel). Deterministlik test PASS (8000 truncat → 16000 retry → parse ok). Jagatud helper `isTruncated(json, parsed)`.
- **Tõestus:** DB kate 100%; unit-test; Osa 54.
