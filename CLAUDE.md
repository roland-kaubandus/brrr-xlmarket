...
7. **Seotud tüübid koos + luku lõpp-terviklikkus** (kas jäi 2 kohta? dup/orphan 0?).
   ...
   - **🔬 GRAB-BAG tuvastus EI ole inv-is** (keyword recall 0% — tõestatud vale-negatiiv, jagatud dominant-sõna maskeerib): `node scripts/grab-bag-judge.mjs` (LLM-semantiline, ~100% recall, perioodiline — uue maini/feedi järel). Vt `reports/detektor-valideerimine.md`. **Õppetund: nime/märksõna-põhine detektsioon EI TÖÖTA semantika jaoks — vajab LLM-judge'i.**
