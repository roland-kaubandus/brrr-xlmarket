...
const RULES_DIR = path_1.default.resolve(process.cwd(), "src/taxonomy/rules");
const L1L2_PATH = path_1.default.join(RULES_DIR, "l1-l2-overrides.json");
// ── helpers ────────────────────────────────────────────────────────
async function withPg(fn) {
    const c = new pg_1.Client(PG_CONFIG);
    await c.connect();
    try {
        return await fn(c);
    }
...
async function resolveCategoryId(c, handle) {
    const r = await c.query(`SELECT id FROM product_category
     WHERE handle = $1 AND deleted_at IS NULL AND is_active = true
     LIMIT 1`, [handle]);
    return r.rows[0]?.id || null;
}
async function movProductToCategory(c, productId, targetCategoryId) {
    // Replace the product's primary category link
    await c.query(`DELETE FROM product_category_product WHERE product_id = $1`, [productId]);
    await c.query(`INSERT INTO product_category_product (product_category_id, product_id)
     VALUES ($1, $2)
     ON CONFLICT DO NOTHING`, [targetCategoryId, productId]);
    // Flip status back to published (if it was draft because of the bucket)
    await c.query(`UPDATE product SET status = 'published', updated_at = NOW()
...
const GET = async (req, res) => {
    ...
    try {
        if (view === "paths") {
            const rows = await withPg(c => c.query(`SELECT raw_path, product_count, first_seen, last_seen
           FROM v_review_queue_top_paths LIMIT $1`, [limit]).then(r => r.rows));
            return res.json({ paths: rows });
        }
        if (view === "stats") {
            const stats = await withPg(async (c) => {
                const open = await c.query(`SELECT COUNT(*)::int AS n FROM v_review_queue`);
                const byMethod = await c.query(`SELECT method, COUNT(*)::int AS n
           FROM category_classification_audit
           WHERE needs_review = true AND resolved_at IS NULL
           GROUP BY method ORDER BY n DESC`);
                const agedOut = await c.query(`SELECT COUNT(*)::int AS n FROM v_review_queue
        ...
        const rows = await withPg(c => c.query(`SELECT q.id, q.product_id, q.method, q.confidence, q.l1_slug, q.l2_slug,
         ...
         ORDER BY q.created_at DESC LIMIT $1`, [limit]).then(r => r.rows));
    ...
    try {
        if (action === "assign") {
            ...
            const { productId, categoryId } = await withPg(async (c) => {
                ...
                if (!catId)
                    throw new Error(`Category not found: ${targetHandle}`);
                const audit = await c.query(`SELECT product_id FROM category_classification_audit WHERE id = $1`, [audit_id]);
                const pid = audit.rows[0]?.product_id;
                if (!pid)
                    throw new Error(`audit row ${audit_id} has no product_id`);
                await movProductToCategory(c, pid, catId);
                await c.query(`UPDATE category_classification_audit
           SET needs_review = false, resolved_at = NOW()
           WHERE id = $1`, [audit_id]);
                await c.query(`INSERT INTO category_classification_audit
             (product_id, action, method, confidence, l1_slug, l2_slug, l3_slug,
              raw_path, needs_review, reason)
           VALUES ($1, 'review_resolved', 'manual_admin', 1.00, $2, $3, $4, $5, false, 'manual assign')`, [pid, l1_slug, l2_slug || null, l3_slug || null, null]);
        ...
        if (action === "dismiss") {
            const { audit_id } = body;
            if (!audit_id)
                return res.status(400).json({ message: "audit_id required" });
            await withPg(c => c.query(`UPDATE category_classification_audit
           SET needs_review = false, resolved_at = NOW()
           WHERE id = $1`, [audit_id]));
        ...
        if (action === "create_rule") {
            ...
            const json = JSON.parse(fs_1.default.readFileSync(L1L2_PATH, "utf-8"));
            json.overrides = json.overrides || {};
            json.overrides[key] = target_slug;
            json._updated = new Date().toISOString().slice(0, 10);
            fs_1.default.writeFileSync(L1L2_PATH, JSON.stringify(json, null, 2) + "\n");
            ...
            const drained = await withPg(c => c.query(`UPDATE category_classification_audit
           SET needs_review = false, resolved_at = NOW()
           WHERE needs_review = true AND resolved_at IS NULL
             AND raw_path LIKE $1
           RETURNING id`, [`${prefix}%`]).then(r => r.rows.length));
